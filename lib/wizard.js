...
function stepAuthToken(rl, config) {
    return question(rl, "初次见面！请输入Bot API token: ")
    .then(function (token) {
        token = token.trim();
...
function stepOwner(rl, config, getNextMessage) {
    ...
    return getNextMessage().then(function (msg) {
        var prompt = util.format("这个 %s «%s» (%s) 是你吗 [y/n]: ", msg.chat.type, msg.chat.name, msg.chat.id);
